	...
	public class IAPProductButton : MonoBehaviour
	{
		...
		private void UpdateButton()
		{
			...
			if (IAPManager.Instance.IsInitialized)
			{
				...
				if (AssignedProductInfo == null)
				{
					Logger.LogError(LogTag, "IAP Product Button does not have a product id assigned to it.");
				...
				else
				{
					...
					if (product == null)
					{
						Logger.LogError(LogTag, "Product with id \"" + productInfo.productId + "\" does not exist.");
						SetTitleText(productInfo.productId + ": does not exist");
					}
					else if (!product.availableToPurchase)
					{
						Logger.LogError(LogTag, "Product with id \"" + productInfo.productId + "\" is not available to purchase.");
